...
NvEncoder::NvEncoder(NV_ENC_DEVICE_TYPE eDeviceType, void *pDevice, uint32_t nWidth, uint32_t nHeight, NV_ENC_BUFFER_FORMAT eBufferFormat,
					 uint32_t nExtraOutputDelay, bool bMotionEstimationOnly) : m_pDevice(pDevice),
...
void NvEncoder::LoadNvEncApi()
{
	...
	{
		char buffer[256];
		sprintf(buffer, "Current Driver Version does not support this NvEncodeAPI version, please upgrade driver. maxSupported=%x", version);
		NVENC_THROW_ERROR(buffer, NV_ENC_ERR_INVALID_VERSION);
	}
